 ...
 */
typedef NS_ENUM(NSInteger, CCLabelVerticalAlignment) {
    CCLabelVerticalAlignmentCenter   = 0,
    CCLabelVerticalAlignmentTop      = 1,
    CCLabelVerticalAlignmentBottom   = 2,
};
...
@protocol CCLabelDelegate;

// Override UILabel @property to accept both NSString and NSAttributedString
@protocol CCLabel <NSObject>
...
/**
 `CCLabel` is a drop-in replacement for `UILabel` that supports `NSAttributedString`, as well as automatically-detected and manually-added links to URLs, addresses, phone numbers, and dates.
 
 ## Differences Between `CCLabel` and `UILabel`
 
 For the most part, `CCLabel` behaves just like `UILabel`. The following are notable exceptions, in which `CCLabel` may act differently:
 ...
 `CCLabel`, like `UILabel`, conforms to `NSCoding`. However, if the build target is set to less than iOS 6.0, `linkAttributes` and `activeLinkAttributes` will not be encoded or decoded. This is due to an runtime exception thrown when attempting to copy non-object CoreText values in dictionaries.
 ...
 */
@interface CCLabel : UILabel <CCLabel, UIGestureRecognizerDelegate>
...
/**
 ...
 @discussion A `CCLabel` delegate responds to messages sent by tapping on links in the label. You can use the delegate to respond to links referencing a URL, address, phone number, date, or date with a specified time zone and duration.
 */
@property (nonatomic, unsafe_unretained) IBOutlet id <CCLabelDelegate> delegate;
...
/**
 The vertical text alignment for the label, for when the frame size is greater than the text rect size. The vertical alignment is `CCLabelVerticalAlignmentCenter` by default.
 */
@property (nonatomic, assign) CCLabelVerticalAlignment verticalAlignment;
...
/**
 Adds a link. You can customize an individual link's appearance and accessibility value by creating your own @c CCLabelLink and passing it to this method. The other methods for adding links will use the label's default attributes.
 
 @warning Modifying the link's attribute dictionaries must be done before calling this method.
 
 @param link A @c CCLabelLink object.
 */
- (void)addLink:(CCLabelLink *)link;
 ...
 */
- (CCLabelLink *)addLinkWithTextCheckingResult:(NSTextCheckingResult *)result;
...
/**
 ...
 @return The newly added link object.
 */
- (CCLabelLink *)addLinkToURL:(NSURL *)url
...
/**
 ...
 @return The newly added link object.
 */
- (CCLabelLink *)addLinkToAddress:(NSDictionary *)addressComponents
                                   withRange:(NSRange)range;
...
/**
 ...
 @return The newly added link object.
 */
- (CCLabelLink *)addLinkToPhoneNumber:(NSString *)phoneNumber
                                       withRange:(NSRange)range;
...
/**
 ...
 @return The newly added link object.
 */
- (CCLabelLink *)addLinkToDate:(NSDate *)date
                                withRange:(NSRange)range;
...
/**
 ...
 @return The newly added link object.
 */
- (CCLabelLink *)addLinkToTransitInformation:(NSDictionary *)components
                                              withRange:(NSRange)range;
...
/**
 Returns the @c CCLabelLink at the give point if it exists.
 
 @discussion This can be used together with @c UIViewControllerPreviewingDelegate to peek into links.
 
 @param point The point inside the label.
 */
- (CCLabelLink *)linkAtPoint:(CGPoint)point;

@end

/**
 The `CCLabelDelegate` protocol defines the messages sent to an attributed label delegate when links are tapped. All of the methods of this protocol are optional.
 */
@protocol CCLabelDelegate <NSObject>
 ...
 */
- (void)attributedLabel:(CCLabel *)label
...
/**
 ...
 @discussion This method is called if no other delegate method was called, which can occur by either now implementing the method in `CCLabelDelegate` corresponding to a particular link, or the link was added by passing an instance of a custom `NSTextCheckingResult` subclass into `-addLinkWithTextCheckingResult:`.
...
@interface CCLabelLink : NSObject <NSCoding>

typedef void (^CCLabelLinkBlock) (CCLabel *, CCLabelLink *);
...
/**
 ...
 @c CCLabelDelegate tap methods, which will not be called for this link.
 */
@property (nonatomic, copy) CCLabelLinkBlock linkTapBlock;

/**
 A block called when this link is long-pressed.
 If non-nil, long pressing on this link will call this block instead of the
 @c CCLabelDelegate long press methods, which will not be called for this link.
 */
@property (nonatomic, copy) CCLabelLinkBlock linkLongPressBlock;
...
/**
 ...
 @return The initialized link object.
 */
- (instancetype)initWithAttributesFromLabel:(CCLabel*)label
                         textCheckingResult:(NSTextCheckingResult *)result;
